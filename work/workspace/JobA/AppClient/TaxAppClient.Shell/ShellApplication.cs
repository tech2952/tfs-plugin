...
namespace TaxAppClient.Shell
{
    ...
    /// </summary>
    public class ShellApplication : FormShellApplication<ShellWorkItem, ShellMainForm>
    {
        ...
        {
            SetProfileCatalogForTestHarness();
        ...
        private static void SetProfileCatalogForTestHarness()
        {
            //first thing this shell needs to do is make sure the right ProfileCatalog.xml is there.  
            FileInfo fi = new FileInfo(Path.Combine(Environment.CurrentDirectory, "ProfileCatalog.xml"));
            FileInfo fiTestHarnessProfileCatalog = new FileInfo(Path.Combine(Environment.CurrentDirectory, "ProfileCatalog.TestHarness.xml"));
            if (fiTestHarnessProfileCatalog.Exists)
            {
                if (fi.Exists)
                {
                    FileInfo fiSave = new FileInfo(Path.Combine(Environment.CurrentDirectory, "ProfileCatalog.Save.xml"));
                    if (fiSave.Exists)
                        fiSave.Delete();
                    fiSave = null;
                    fi.MoveTo(Path.Combine(Environment.CurrentDirectory, "ProfileCatalog.Save.xml"));
                    fiTestHarnessProfileCatalog.CopyTo(Path.Combine(Environment.CurrentDirectory, "ProfileCatalog.xml"));
                }
            }
        }
