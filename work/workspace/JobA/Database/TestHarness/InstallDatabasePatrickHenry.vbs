...
'[InstallDir]
'    	--> TaxBuilderRuntimeTables.sql
'       --> TaxBuilderRuntimeProcs.sql
...
On Error Resume Next
	...
	Set logFile = fsoLog.CreateTextFile ( strTempDir & "\PatrickHenry_Database_Install.txt", TRUE)
	...
	if err.Description = "" then
		...
		intReturn = MsgBox ("Database Exists: Remove?", vbYesNo)
		if intReturn = 6 then
	...
	if not bError then
		if not DatabaseExists then
			RunSQLCommand "UserDefinedDatatypes.sql", "PatrickHenry_UDT.txt"
			logFile.WriteLine "Create Tables script ran, please view REVERE_UserDDT.txt for output"
		end if
	end if
	...
	if not bError then
		if not DatabaseExists then
			RunSQLCommand "TaxBuilderRuntimeTables.sql", "PatrickHenry_Tables.txt"
			logFile.WriteLine "Create Tables script ran, please view PatrickHenry_Tables.txt for output"
		end if
	end if
	...
	if not bError then
		if not DatabaseExists then
			RunSQLCommand "TaxBuilderRuntimeProcs.sql", "PatrickHenry_Procs.txt"
			logFile.WriteLine "Create Procs script ran, please view PatrickHenry_Procs.txt for output"
		end if
	end if
