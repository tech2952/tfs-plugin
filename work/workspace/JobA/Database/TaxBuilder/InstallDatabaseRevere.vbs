...
On Error Resume Next
	...
	Set logFile = fsoLog.CreateTextFile ( strTempDir & "\REVERE_Database_Install.txt", TRUE)
	...
	if not bError then
		if not DatabaseExists then
			RunSQLCommand "UserDefinedDatatypes.sql", "REVERE_UserDDT.txt"
	...
	if not bError then
		if not DatabaseExists then
			RunSQLCommand "tables.sql", "REVERE_Tables.txt"
			logFile.WriteLine "Create Tables script ran, please view REVERE_Tables.txt for output"
		end if
	end if
	...
	'---------------------------------------------
	if not bError then
		RunSQLCommand "procs.sql", "REVERE_Procs.txt"
