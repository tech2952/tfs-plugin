...
strDebug = "true"
strDatabaseName = "PAULREVERE"
strImageDatabaseName = "TaxBuilderImageDatabase"
strServerName = "(local)\sqlexpress"
strUserName = "sa"
...
'On Error Resume Next
	...
	'----------------------------------------------
	Set logFile = fsoLog.CreateTextFile ( strRunDir & "\REVERE_Call_Database_Install.txt", TRUE)
	...
	if strSourceDir = "" then
		strSourceDir = Left(WScript.ScriptFullName, Len(WScript.ScriptFullName) - 32)
	end if
	...
	'--------------------------------------------
	strInstallDatabase = strSourceDir + "InstallDatabaseRevere.vbs"
	...
	'-------------------------------------------------------------------
	'------- Launch Installscript for the Image Database ---------------
	'-------------------------------------------------------------------
	strInstallDatabase = strSourceDir + "InstallTaxBuilderImageDatabase.vbs"
	strFlags = Flag1 & Flag2 & Flag3 & Flag4 & Flag5
	logFile.WriteLine "Flags: " & strFlags
	strCmd = """" & strInstallDatabase & """" & " " & _
			strServerName & " " & _
			strImageDatabaseName & " " & _
			"""" & strSourceDir & """" & " " & _
			"""" & strDatabaseDir & """" & " " & _
			"""" & strFlags & """" & " " & _
			"""" & strUserName & """" & " " & _
