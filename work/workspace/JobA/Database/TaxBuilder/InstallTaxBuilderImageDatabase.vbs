...
'[InstallDir]
'    	--> TaxBuilderDesignerImageTables.sql
...
On Error Resume Next
	...
	Set logFile = fsoLog.CreateTextFile ( strTempDir & "\REVEREImage_Database_Install.txt", TRUE)
...
'		end if
	...
	if not bError then
		if not DatabaseExists then
			RunSQLCommand "UserDefinedDatatypes.sql", "REVEREImage_UserDDT.txt"
			logFile.WriteLine "Create Tables script ran, please view REVEREImage_UserDDT.txt for output"
		end if
	end if
	...
	if not bError then
		if not DatabaseExists then
			RunSQLCommand "TaxBuilderDesignerImageTables.sql", "REVEREImage_Tables.txt"
			logFile.WriteLine "Create Tables script ran, please view REVEREImage_Tables.txt for output"
		end if
	end if
	...
	'---------------------------------------------
'	if not bError then
'		RunSQLCommand "procs.sql", "REVEREImage_Procs.txt"
'		logFile.WriteLine "Create Stored Procedures script ran, please view REVEREImage_Procs.txt for output"
'	end if
